...
class WiKG(nn.Module):
    def __init__(self, dim_in=384, dim_hidden=512, topk=6, n_classes=1, agg_type='bi-interaction', dropout=0.3,
                 pool='mean'):
        super().__init__()
...
if __name__ == "__main__":
    ...
    model = WiKG(dim_in=384, dim_hidden=512, topk=6, n_classes=2, agg_type='bi-interaction', dropout=0.3,
                 pool='attn').cuda()
    output = model(data)
    print(output.shape)
